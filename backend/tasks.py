...
def create_logistics_task(logistics_manager) -> Task:
    ...
    return Task(
        description=(
            ...
            "1. **Travel (Flights) -- ONE-WAY:** You MUST use the `FlightSearchTool` to fetch real flight options. "
            ...
            "   *Note: If flights are not applicable (e.g., short distances within the same country), use your general Search tool to find estimated Train/Bus prices and explicitly label them as '(Estimated)'*\n"
            "   For EACH option, provide:\n"
            "   - **Route:** {origin} to {destination} -- Airline name, price PER PERSON, duration, and number of stops.\n"
            ...
            "   - **[BUDGET] Package:** Cheapest ONE-WAY flight x {num_persons} + budget hotel + daily expenses x {num_persons} = TOTAL\n"
            "   - **[MID-RANGE] Package:** Mid-price ONE-WAY flight x {num_persons} + mid-range hotel + daily expenses x {num_persons} = TOTAL\n"
            "   - **[PREMIUM] Package:** Best ONE-WAY flight x {num_persons} + premium hotel + daily expenses x {num_persons} = TOTAL\n\n"
        ...
        expected_output=(
            ...
            "**One-Way Flight / Train Options (comparison table -- price per person):**\n"
