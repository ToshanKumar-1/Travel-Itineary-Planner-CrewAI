...
def create_logistics_task(logistics_manager) -> Task:
    ...
    return Task(
        description=(
            ...
            "Budget: {currency_symbol}{budget_per_person} per person x {num_persons} person(s) = "
            ...
            "1. **Travel (Transport) -- ONE-WAY:** First, evaluate the distance between {origin} and {destination}. "
            "If the distance is short (e.g., reachable by a 2-8 hour train or bus ride), you MUST prioritize finding Train or Bus options using your general Search tool to ensure the trip stays within the strict budget. "
            "If the distance is long, use the `FlightSearchTool` to fetch real flight options. "
            "IMPORTANT: The FlightSearchTool requires IATA airport codes (e.g., DEL for Delhi, JFK for New York, CDG for Paris, NRT for Tokyo). "
            ...
            "   - **[BUDGET] Package:** Cheapest ONE-WAY transport (bus/train/flight) x {num_persons} + budget hotel + daily expenses x {num_persons} = TOTAL\n"
            "   - **[MID-RANGE] Package:** Mid-price ONE-WAY transport x {num_persons} + mid-range hotel + daily expenses x {num_persons} = TOTAL\n"
            "   - **[PREMIUM] Package:** Best ONE-WAY transport x {num_persons} + premium hotel + daily expenses x {num_persons} = TOTAL\n\n"
        ...
        expected_output=(
            ...
            "**One-Way Transport Options (comparison table -- price per person):**\n"
            "| Option | Route | Provider (Airline/Train/Bus) | Class | Price ({currency_symbol}) | Local Currency |\n"
            "| 1 | {origin} to {destination} | [Name] | Economy | {currency_symbol}XXX | local XXX |\n"
            "| 2 | ... | ... | ... | ... | ... |\n"
            "| 3 | ... | ... | ... | ... | ... |\n\n"
            ...
            "| Budget | [Name] | 2-star | {currency_symbol}XXX | local XXX | {currency_symbol}XXX |\n"
            "| Mid-Range | [Name] | 3-star | {currency_symbol}XXX | local XXX | {currency_symbol}XXX |\n"
            "| Premium | [Name] | 4-star | {currency_symbol}XXX | local XXX | {currency_symbol}XXX |\n\n"
            "**Daily Expenses Breakdown (per person per day):**\n"
            "- Breakfast / Lunch / Dinner / Transport\n\n"
            "**Package Comparison (total for {num_persons} person(s), including one-way transport):**\n"
            "| Package | One-way Transport | Hotel | Daily Exp. | TOTAL | Budget Status |\n"
            "| [BUDGET] | ... | ... | ... | {currency_symbol}XXX | Within/Over |\n"
            "| [MID-RANGE] | ... | ... | ... | {currency_symbol}XXX | Within/Over |\n"
...
def create_compilation_task(compiler, context_tasks: list) -> Task:
    ...
    return Task(
        description=(
            ...
            "2. Include the **Transport Options** comparison table from the logistics report -- "
        ...
        expected_output=(
            ...
            "- Transport Options comparison table (5-10 options, dual currency)\n"
